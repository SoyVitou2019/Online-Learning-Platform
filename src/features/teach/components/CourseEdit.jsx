...
const CourseEdit = () => {
  ...
  const fetchCourse = async () => {
    ...
    try {
      ...
      const response = await axios.get(END_POINTS.COURSE + "/" + courseID);

      const responseData = response.data;
      let courseData = {
        courseName: responseData.course_name,
        courseDescription: responseData.couse_description,
        courseExpectations: responseData.course_expectation.join("\n"),
        courseCategory: responseData.category,
      };

      setCourseData(courseData);
      setCurrCourse(responseData);
      const catData = await axios.get(
        END_POINTS.CATEGORY + "?title=" + responseData.category
      );
      console.log(END_POINTS.CATEGORY + "?title=" + responseData.category);
      setSelected(catData.data[0]);

      let vidArr = [];
      await Promise.all(
        responseData.posts.map(async (post, index) => {
          const postDetail = await axios.get(END_POINTS.POST + "/" + post);
          vidArr.push({
            id: index + 1,
            link: "https://www.youtube.com/watch?v=" + postDetail.data.vid_id,
            name: postDetail.data.title,
          });
        })
      );
  ...
  const handleSubmit = async () => {
    ...
    await Promise.all(
      currCourse.posts.map(async (post) => {
        await axios.delete(END_POINTS.POST + "/" + post);
      })
    );
      ...
      {!loading && (
        <div className="mb-32">
          ...
          <div className="px-5 flex justify-start border-t border-b border-gray-300">
            ...
            <Link
              to="/teach/dashboard"
              className=" p-4  py-2 text-md font-medium "
            >
              Dashboard
            </Link>
            <button

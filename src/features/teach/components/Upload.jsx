        ...
        <div className="flex justify-center gap-9">
          ...
          <div className="flex flex-col mt-5 ml-10 items-start">
                    ...
                    <Listbox.Options className="absolute  mt-1 max-h-60 w-full overflow-auto rounded-md bg-pink-100 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                      {people.map((person, personIdx) => (
                        <Listbox.Option
                          key={personIdx}
                          className={({ active }) =>
                            `relative cursor-pointer select-none py-2 pl-10 pr-4 ${active ? 'bg-red-200 text-amber-900' : 'text-gray-900'
                            }`
                          }
                          value={person}
                        >
            ...
            <div className='w-80 m3 ml-4'>
              <label htmlFor="description" className="block mb-2 text-sm font-medium text-black">Description of course</label>
              <textarea id="description" rows="4" className="block p-2.5 w-full text-sm border border-black focus:border-2" placeholder="Write your thoughts here..."></textarea>
            </div>
          </div>
          <div>
          </div>
            ...
            {/* video */}
            {
              youtubeLink.map((item, idx) => (
                <div className='w-80 pl-3 flex select-none' key={idx}>
                  <p className='flex flex-col px-4 text-white items-center bg-white border border-gray-200 shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700'>{idx + 1}</p>
                  <a href="#" className="flex flex-col w-full items-center bg-white border border-gray-200 shadow md:flex-row md:max-w-xl dark:border-gray-700 dark:bg-gray-800">
                    <div className="flex flex-col w-full justify-between p-4 leading-normal">
                      <h5 className="text-sm line-clamp-1 w-full font-bold tracking-tight text-gray-900 dark:text-white">{item.title}</h5>
                    </div>
                  </a>
                </div>
              ))

...
export const ViewProfile = () => {
    ...
    const checkExitEncounter = () => {
        ...
        userFollowData.userId.forEach((item, idx) => {
            if (item === parseInt(creator_id) && userID === userFollowData.follower[idx]) {
    ...
    useEffect(() => {
        ...
        const fetchUserCourse = async () => {
            try {
                const response = await axios.get(`${END_POINTS.COURSE}?created_by_user_id=${creator_id}&_limit=5`);
                const responseData = response.data;
        ...
        <div>
            <div className="bg-white border rounded-lg m-4 ">
                ...
                <div className="flex-col p-8 ">
                    <span className="text-2xl whitespace-nowrap dark:text-black font-bold">{userData.firstName + " " + userData.lastName}</span>
                    <div className="flex gap-7">
                        <h1 className="text-blue-500 flex justify-center items-center text-md underline py-2 select-none">@FuckyouUlittleCunt</h1>
                        {userID !== parseInt(creator_id) ? (
                            isCheckUserFollow ? (
                                <button onClick={() => { unfollow(userID, creator_id) }} className="bg-blue-500 hover:bg-blue-700 flex gap-5 text-white font-bold py-2 px-4 rounded">
                                    <p >Unfollow</p>
                                    <i className="bi bi-balloon-heart-fill"></i>
                                    </button>
                            ) : (
                                <button onClick={() => { follow(creator_id, userID) }} className="bg-blue-500 hover:bg-blue-700 flex gap-5 text-white font-bold py-2 px-4 rounded">
                                    <p >Follow</p>
                    ...
                    <div className="flex justify-between pt-2 w-64">
                        <h1 className="text-black text-md">Role as a {userData.role}</h1>
                        <p className="text-black text-md underline">Contact Info</p>
                    </div>
                    <div className="flex gap-5">
                        <h1 className="text-blue-500 text-md pt-4 underline mb-3">{follower.followerId.length} followers</h1>
                        <h1 className="text-blue-500 text-md pt-4 underline mb-3">{following.followingId.length} followings</h1>
                    </div>
                    <div className="flex gap-8 justify-between pt-2 w-64">

                        <Button className="py-1 bg-blue-500 hover:bg-blue-600">
                            <h1 onClick={() => { setShow_info("view_courses") }} className="text-white">
                                View Courses
                            </h1>
                        </Button>
                        <Button className="py-1 bg-blue-500 hover:bg-blue-600">
                            <h1 onClick={() => { setShow_info("follower") }} className="text-white">
                                Follower
                            </h1>
                        </Button>
                        <Button className="py-1 bg-blue-500 hover:bg-blue-600">
                            <h1 onClick={() => { setShow_info("following") }} className="text-white">
                                Following
                            </h1>
                        </Button>
                ...
                <div className="flex-col p-8 ">
                    {
                        show_info === "view_courses" ? (
                            ...
                            <h1 className="font-bold text-xl">Follower Him</h1>
                        ) : (
                            <h1 className="font-bold text-xl">Following Him</h1>
                        )
                    }
                    <div className="flex justify-between pt-5">
                        {
                            show_info === "view_courses" ? (
                                userCourse.map((item, idx) => (
                                    <div key={idx}>
                                        <img className="border rounded-lg aspect-video w-40" src={`https://img.youtube.com/vi/${item.vid_id}/mqdefault.jpg`} alt={item.course_name} />
                                        <p className="indent-3 line-clamp-1 font-bold">{idx + 1}. {item.course_name}</p>
                                    </div>
                                ))
                            ) : show_info === "follower" ? (
